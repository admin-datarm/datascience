...
#5.sum PM2.5 by year and type.
sum_coal_pm25 <- coal_pm25 %>% group_by(year,type) %>% summarise(Emissions=sum(Emissions))
#6. Plot using ggplot2
png(filename="../airpollution_courseproject2/plot4.png")
ggplot(sum_coal_pm25, aes(x=year, y=Emissions, color=type)) + geom_point() + geom_line()+labs(title="Coal combustion-related sources")
dev.off()
